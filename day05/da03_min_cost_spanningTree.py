...
from operator import itemgetter # 튜플, 리스트의 인덱스에 해당하는 아이템을 가져올 때 사용하는 모듈 
...
def printGraph(g): # g : Graph 객체 매개변수
    ...
    print('\t', end='\t')
    for v in range(g.SIZE): # v -> vertax 매개변수
        print(nameAry[v], end='\t')
    print()
    ...
    while len(stack) != 0: 
        ...
        for vertex in range(SIZE):
            if g.graph[current][vertex] != 0 : # 간선이 있다
                if vertex in visitedAry: # 이미 방문했으면 pass
                    continue # pass도 가능
                else:

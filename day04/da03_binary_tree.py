...
class TreeNode:
    def __init__(self):
        ...
        self.data = None # 노드에 저장된 값
        
        
# 메인 모튤
if __name__ == '__main__':
    node = TreeNode() # 노드생성
    node.data = nameAry[0] #첫번째 노드를 루트노드로 지정
    
    root = node 
    memory.append(node) #생성한 노드를 메모리에 적재
        ...
        while True:
            if name < current.data : # 현재 name이 노드 안 데이터보다 작으면, 왼쪽
                if current.left == None :
                    ...
                    current = current.left # 왼쪽을 더 내려감 
            
            else :# 현재 name이 노드 안 데이터보다 크면, 오른쪽
    ...
    print('이진탐색 트리 구성완료') # for문끝
    
    # 검색
    findName = input('찾을 이름 입력 > ')
    
    current = root #루트노드부터 검색시작
    count  = 0
    ...
    while True:
        if delName == current.data: # 삭제할 때
            ...
            elif current.left != None and current.right == None: #왼쪽 노드에 자식이 있고, 오른쪽에는 없을 때
                if parent.left == current:
